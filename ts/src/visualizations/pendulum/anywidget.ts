...
import { initPendulumVisualization } from './index';
...
async function render({
  ...
  parent.id = 'pendulum-visualization';
  ...
  const mount = async(): Promise<void> => {
    ...
    const created = await initPendulumVisualization(parent, weightsUrl);
    if (currentGeneration !== generation) { created.destroy(); return; }
    visualization = created;
  };

  const onWeightsBase64Change = (): void => { void mount(); };
